    ...
    <section className="relative mx-auto max-w-6xl py-12">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
        <div>
          ...
          <p className="text-gray-600 mt-2 max-w-xl">
            Cast your vote in real-time – every voice matters. See the latest
            active elections below.
          </p>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {filtered.map((election, index) => {
          const total =
            election.maxVoters || election.totalVotes || 1;

          const progress = Math.min(
            100,
            Math.round((election.totalVotes / total) * 100)
          );

          const color = colors[index % colors.length];

          const remaining =
            timeLeft[election.id] || {
              total: 0,
              days: 0,
              hours: 0,
              minutes: 0,
          ...
          return (
            ...
            >
              <div
                className={`absolute inset-0 opacity-0 group-hover:opacity-5 transition duration-500 rounded-2xl bg-gradient-to-br ${color}`}
              />
