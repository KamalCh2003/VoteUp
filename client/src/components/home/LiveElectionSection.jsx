  ...
  return (
    <section className="relative mx-auto max-w-6xl py-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
        ...
        <Link
          to="/elections"
          className="flex items-center gap-1 text-sm text-violet-400 hover:text-violet-300 transition group"
        >
          View all elections
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <div className="mt-4 flex items-center justify-between text-xs text-zinc-400">
                  <span>{election.totalVotes?.toLocaleString()} / {total.toLocaleString()} votes</span>
                  <span>{progress}% turnout</span>
                </div>
              </div>
            </Link>
          );
      ...
      {filtered.length === 0 && (
        <p className="text-zinc-400 text-center mt-10">No active elections found.</p>
      )}
    </section>
  );
}